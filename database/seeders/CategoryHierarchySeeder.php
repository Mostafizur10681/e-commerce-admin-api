...
class CategoryHierarchySeeder extends Seeder
{
    ...
    {
        $men = Category::firstOrCreate(['name' => 'Men'], ['status' => true]);
        $women = Category::firstOrCreate(['name' => 'Women'], ['status' => true]);
        $children = Category::firstOrCreate(['name' => 'Children'], ['status' => true]);

        // Assign sub-categories under main categories
        Category::whereIn('id', [1, 3, 4])->update(['parent_id' => $men->id]);
