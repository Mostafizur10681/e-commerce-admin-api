...
class CategoryService
{
    ...
    {
        return $this->categoryRepository->getActive(['parent', 'children', 'subCategories']);
    }

    public function paginateCategories(int $perPage = 15): LengthAwarePaginator
    {
        return $this->categoryRepository->paginateActive($perPage, ['parent', 'children', 'subCategories']);
    }

    public function getCategoryById(int|string $id): ?Model
    {
        return $this->categoryRepository->findOrFail($id, ['*'], ['parent', 'children', 'subCategories']);
    }
