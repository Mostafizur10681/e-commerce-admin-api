...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div>
        ...
        <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white mt-1">Edit Attribute: {{ $attribute->name }}</h1>
    </div>

    <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 shadow-sm">
        <form method="POST" action="{{ route('admin.attributes.update', $attribute->id) }}" class="space-y-4" x-data="{
            tagInput: '',
            values: {!! json_encode($existingValues) !!},
            addTag() {
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $attribute->name) }}" required class="w-full px-3.5 py-2.5 bg-gray-50 dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-xl text-xs text-gray-900 dark:text-white focus:outline-none focus:border-primary">
            </div>
            ...
            <div>
                ...
                <select name="type" required class="w-full px-3.5 py-2.5 bg-gray-50 dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-xl text-xs text-gray-900 dark:text-white focus:outline-none focus:border-primary">
                    ...
                    <option value="button" {{ old('type', $attribute->type) === 'button' ? 'selected' : '' }}>Button Pills</option>
            ...
            <div class="pt-4 flex items-center justify-end gap-3 border-t border-gray-100 dark:border-gray-800">
                ...
                <button type="submit" class="px-5 py-2.5 bg-primary hover:bg-primary-hover text-white text-xs font-bold rounded-xl shadow-md shadow-primary/20">Update Attribute</button>
            </div>
        </form>
    </div>
