...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div>
        ...
        <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white mt-1">Edit Category: {{ $category->name }}</h1>
    </div>
    ...
    <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 shadow-sm">
        <form method="POST" action="{{ route('admin.categories.update', $category->id) }}" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                ...
                <textarea name="description" rows="3" class="w-full px-3.5 py-2 bg-gray-50 dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-xl text-xs text-gray-900 dark:text-white focus:outline-none focus:border-primary">{{ old('description', $category->description) }}</textarea>
            </div>
            ...
            @if($category->image)
                <div>
                    ...
                    <div class="h-16 w-16 rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700 bg-gray-50">
                        <img src="{{ str_starts_with($category->image, 'http') ? $category->image : asset('storage/' . $category->image) }}" class="h-full w-full object-cover">
                    </div>
                </div>
            @endif
            ...
            <div class="pt-2">
                <label class="flex items-center gap-2.5 cursor-pointer text-xs text-gray-700 dark:text-gray-300">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $category->is_active) ? 'checked' : '' }} class="rounded border-gray-300 text-primary focus:ring-primary h-4 w-4">
