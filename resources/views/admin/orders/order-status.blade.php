...
<div class="space-y-6">
    ...
    <div>
        ...
        <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white mt-1">Order Status Configuration</h1>
        <p class="text-xs text-gray-500 dark:text-gray-400">Manage fulfillment progression stages.</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-5 shadow-sm space-y-4">
            <h3 class="text-sm font-bold text-gray-900 dark:text-white border-b border-gray-100 dark:border-gray-800 pb-3">Add Order Status</h3>
            <form method="POST" action="{{ route('admin.orders.order-status.store') }}" class="space-y-3.5">
                @csrf
                <div>
                    <label class="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Status Name *</label>
                    <input type="text" name="name" required placeholder="e.g. Processing, Shipped, Ready for Pickup" class="w-full px-3.5 py-2.5 bg-gray-50 dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-xl text-xs text-gray-900 dark:text-white focus:outline-none focus:border-primary">
                </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-xs">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="px-5 py-8 text-center text-gray-400">No order statuses configured.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
