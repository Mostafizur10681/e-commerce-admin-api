...
@section('content')
<div x-data="paymentStatusManager()" class="space-y-6 max-w-7xl mx-auto pb-16">
    ...
    <div class="space-y-1">
        <div class="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400 font-medium">
            ...
            <span>&gt;</span>
            <span class="text-slate-800 dark:text-slate-200 font-semibold">Payment Statuses</span>
        </div>
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pt-1">
            <div class="flex items-center gap-3">
                <h1 class="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Payment Statuses</h1>
            ...
            <div class="flex items-center gap-2">
                <a href="{{ route('admin.orders.order-status') }}" class="inline-flex items-center gap-1.5 text-xs font-semibold px-3.5 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-all">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <!-- LEFT COLUMN: ADD PAYMENT STATUS CARD -->
        <div class="lg:col-span-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm space-y-4 lg:sticky lg:top-6">
            <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
                <div>
                    <h2 class="text-sm font-bold text-slate-900 dark:text-white">Add Payment Status</h2>
                    <p class="text-[11px] text-slate-400">Configure new transaction state badge.</p>
                </div>
            ...
            <form method="POST" action="{{ route('admin.orders.payment-status.store') }}" class="space-y-4">
                @csrf
                <div>
                    ...
                    <input 
                        ...
                        name="name" 
                        required 
                        placeholder="e.g. Paid, Pending, Refunded, Partially Paid" 
                        value="{{ old('name') }}"
                ...
                <div>
                    ...
                    <textarea 
                        name="description" 
                        rows="2" 
                        placeholder="Brief note about when this payment state applies..." 
        ...
        <div class="lg:col-span-8 space-y-4">
            ...
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 shadow-sm space-y-4">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div class="relative flex-1">
                        ...
                        <input 
                            type="text" 
                            x-model="searchQuery" 
                            placeholder="Filter payment statuses..." 
                ...
                <div class="hidden lg:block overflow-x-auto border border-slate-100 dark:border-slate-800/80 rounded-2xl">
                    <table class="w-full text-left text-xs">
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-800/80">
                                ...
                                >
                                    ...
                                    <td class="px-5 py-4 text-right">
                                        <div class="flex items-center justify-end gap-1.5">
                                            ...
                                            <!-- Delete Button -->
                                            <form method="POST" action="{{ route('admin.orders.payment-status.destroy', $item->id) }}" onsubmit="return confirm('Are you sure you want to delete payment status \'{{ addslashes($item->name) }}\'?');" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button 
                                                    type="submit" 
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="px-5 py-8 text-center text-slate-400 italic">No payment statuses configured.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:hidden">
                        ...
                        >
                            ...
                            <div class="pt-2 border-t border-slate-200/80 dark:border-slate-800 flex items-center justify-between gap-2">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <form method="POST" action="{{ route('admin.orders.payment-status.destroy', $item->id) }}" onsubmit="return confirm('Delete payment status \'{{ addslashes($item->name) }}\'?');" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button 
                                            type="submit" 
                    ...
                    @empty
                        <div class="col-span-full py-8 text-center text-slate-400 italic">No payment statuses configured.</div>
                    @endforelse
                </div>
        ...
        >
            <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
                <div class="flex items-center gap-2.5">
                    ...
                    <h3 class="text-sm font-bold text-slate-900 dark:text-white">Edit Payment Status</h3>
                </div>
            ...
            <form :action="'/admin/orders/payment-status/' + editForm.id" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
